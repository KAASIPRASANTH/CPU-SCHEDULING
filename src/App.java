...
//import org.cpuschedule.MsgBox;
import algo.*;
import utility.InputData;
import utility.OutputData;
import utility.Pair;
...
public class App extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        fcfs_button.setMaxWidth(Double.MAX_VALUE);
        fcfs_button.setMinHeight(100);
        fcfs_button.setOnAction(e -> algo_btn_click(1));

        sfj_button.setMaxWidth(Double.MAX_VALUE);
        sfj_button.setMinHeight(100);
        sfj_button.setOnAction(e -> algo_btn_click(2));

        rr_button.setMaxWidth(Double.MAX_VALUE);
        rr_button.setMinHeight(100);
        rr_button.setOnAction(e -> algo_btn_click(3));

        ps_button.setMaxWidth(Double.MAX_VALUE);
        ps_button.setMinHeight(100);
    ...
    public void set_inptable(int algo) {
                ...
                new EventHandler<TableColumn.CellEditEvent<InputData, Integer>>() {
                    ...
                    public void handle(TableColumn.CellEditEvent<InputData, Integer> edit) {
                        (edit.getTableView().getItems().get(
                                edit.getTablePosition().getRow())).setPid(edit.getNewValue());
                    }
                });
        ...
        col_btime.setOnEditCommit(
                new EventHandler<TableColumn.CellEditEvent<InputData, Integer>>() {
                    ...
                    public void handle(TableColumn.CellEditEvent<InputData, Integer> edit) {
                        (edit.getTableView().getItems().get(
                                edit.getTablePosition().getRow())).setBtime(edit.getNewValue());
                    }
                });
        ...
        if (algo == 4) {
            ...
            col_prior.setOnEditCommit(
                    new EventHandler<TableColumn.CellEditEvent<InputData, Integer>>() {
                        ...
                        public void handle(TableColumn.CellEditEvent<InputData, Integer> edit) {
                            (edit.getTableView().getItems().get(
                                    edit.getTablePosition().getRow())).setPriority(edit.getNewValue());
                        }
                    });
    ...
    public void submit_data(int algo) {
        ...
        if (table.getItems().size() == Integer.parseInt(input_n.getText())) {
            ...
            for (InputData data : rows) {
                ...
                if (algo == 4) {
                    input_data.add(data.getPriority());
                }
            }
            switch (algo) {
                case 1:
